 ...
 */
 ...
 func (h *HTTPInboundEndpoint) Start(ctx context.Context, mediator ports.InboundMessageMediator) error {
	 ...
	 portStr, exists := h.config.Parameters["inbound.http.port"]
	 if !exists {
		 h.logger.Error("Port configuration missing", "parameter", "inbound.http.port")
		 return errors.New("inbound HTTP port configuration missing")
	 }
 
	 // Convert port to integer
	 inboundServerPort, err := strconv.Atoi(portStr)
	 if err != nil {
		 h.logger.Error("Invalid port configuration", "port", portStr, "error", err)
	 ...
	 configCtx, ok := ctx.Value(utils.ConfigContextKey).(artifacts.ConfigContext)
	 if !ok {
		 h.logger.Warn("Config context not found in context, using default configuration")
		 return errors.New("config context not found in context")	
	 }
	 ...
	 if serverConfig, ok := configCtx.DeploymentConfig["server"].(map[string]string); ok {
		 ...
		 if offsetStr, offsetExists := serverConfig["offset"]; offsetExists {
			 if offsetInt, err := strconv.Atoi(offsetStr); err == nil {
				 ...
				 h.logger.Info("Using port offset: %d, final port: %d", offsetInt, inboundServerPort)
			 } else {
